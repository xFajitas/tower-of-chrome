...
public class InitiativeTests
{
    ...
    public void EmptyOrAllDead_ReturnsEmptyList()
    {
        ...
        Assert.Empty(Initiative.RollInitiative(Array.Empty<TowerOfChrome.Core.Entities.Combatant>(), rng));
    }
    ...
    public void InputList_IsNotMutated()
    {
        ...
        var input = new List<TowerOfChrome.Core.Entities.Combatant> { a, b };
        var rng = new ScriptedRandomSource(ints: new[] { 1, 1 }, doubles: new[] { 0.1, 0.1 });

        Initiative.RollInitiative(input, rng);

        Assert.Equal(new TowerOfChrome.Core.Entities.Combatant[] { a, b }, input); // original order untouched
    }
}
